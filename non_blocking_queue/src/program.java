...
public class program {
	...
	private static void testQueue() throws Exception {
		Queue q = new SimpleQueue2(1024 * 1024 * 16); 
//		Queue q = new RingBuffer(1024 * 1024 * 16); 
//		 Queue q = new RingBufferV2(1024 * 1024 * 16);
//		 Queue q = new RingBufferV3(1024 * 1024 * 16);
	...
	private static long test(int noThreads, int noItems, Queue queue) throws InterruptedException {
		...
		long counter = 0;
		for (int i = 0; i < consumers.length; i++) {
			counter += consumers[i].getCounter();
		}
